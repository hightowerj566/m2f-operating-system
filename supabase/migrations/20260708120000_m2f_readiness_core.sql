-- M2F OS · Migration 1 · Readiness Core
-- Categories, assessment CMS, versioned score snapshots, profile clock, lead payload.

-- 1. The seven locked categories
...
CREATE POLICY "Categories are publicly readable"
  ON public.readiness_categories FOR SELECT USING (true);
...
-- 2. Assessment content as data (Appendix A)
...
-- 3. Versioned score snapshots (the spine)
...
CREATE POLICY "Users read own category scores"
  ON public.assessment_category_scores FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.assessments a WHERE a.id = assessment_id AND a.user_id = auth.uid()));
CREATE POLICY "Users insert own category scores"
  ON public.assessment_category_scores FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.assessments a WHERE a.id = assessment_id AND a.user_id = auth.uid()));

-- 4. The clock, on the profile
...
-- 5. Lead payload for the score-gap email sequence
...
-- 6. Seed Appendix A (fixed UUIDs so options can reference questions)
